...
def update_or_create_crawl_detail(data):
    # Updated to timezone.now()
    defaults_data = {"fetch_datetime": timezone.now(), **data}
...
def update_or_create_detail_info(detail_data):
    try:
        ...
        company_instance, company_created = custom_update_or_create(
            Company, defaults=company_data, name=company_data["name"], address=company_data["address"],)
