...
const submitJigyosyoTransaction = (requestMethod) => async (
  e,
  formData,
  navigator,
  id = undefined
  ...
  e.preventDefault();
  const { navigate, setOpenSnackbar, setSnackbarMessage, setSnackbarSeverity } =
    navigator;
  ...
  const dataToSubmit = {
    ...
    koyou_care_services_consult: formData.koyou_care_services_consult || false,
  ...
  try {
    await axiosInstance[requestMethod](`jigyosyo-transaction/${id}/`, dataToSubmit);
