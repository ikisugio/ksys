import {
  KEIKEI_KUBUN_CHOICES,
  SUPPORT_STATUS_CHOICES,
  SUPPORT_MEANS_CHOICES,
} from "./label-choices";

export const TRANSACTION_FIELDS = [
  // CharField, DateField, TextField, FileField
  {
    name: "management",
    label: "管理",
    isDisplay: false,
    type: "select",
    options: KEIKEI_KUBUN_CHOICES,
  },
  { name: "visit_date", label: "訪問日", isDisplay: false, type: "date" },
  { name: "visit_memo", label: "訪問メモ", isDisplay: false, type: "text" },
  { name: "file", label: "ファイル", isDisplay: false, type: "file" },
  { name: "history", label: "履歴", isDisplay: false, type: "text" },

  // CharField with choices
  {
    name: "keikei_kubun",
    label: "系型区分",
    isDisplay: false,
    type: "select",
    options: KEIKEI_KUBUN_CHOICES,
  },
  {
    name: "support_status",
    label: "サポート状況",
    isDisplay: false,
    type: "select",
    options: SUPPORT_STATUS_CHOICES,
  },
  {
    name: "support_means",
    label: "サポート手段",
    isDisplay: false,
    type: "select",
    options: SUPPORT_MEANS_CHOICES,
  },

  // BooleanField
  {
    name: "is_recruiting_on_hw",
    label: "ＨＷに募集中",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "is_recruiting_on_expect_hw",
    label: "ＨＷ以外に募集予定",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "is_going_to_recruit",
    label: "募集予定",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "is_accepting_intern",
    label: "介護実習の受入",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "will_inform_hw",
    label: "労働局（ＨＷ）への伝達",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "will_inform_prefecture",
    label: "都道府県への伝達",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "will_inform_others",
    label: "その他の関係機関への伝達",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "done_explain_support",
    label: "支援メニューの説明",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "done_knowing_problem",
    label: "課題の把握",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_tool_utilization",
    label: "ツールの活用",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_employment_consultant",
    label: "雇用コンサルと同行",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_health_counselor",
    label: "ヘルスカウンセラーと同行",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_training_coordinator",
    label: "研修コーディネーターと同行",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_alone_on_hw",
    label: "労働局委託で単独",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "with_staff_on_hw",
    label: "労働局委託で職員と同行",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_job_posting_consult",
    label: "雇用管理にかかる求人求職の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_job_posting_inform",
    label: "雇用管理にかかる求人求職の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_working_conditions_consult",
    label: "労働条件の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_working_conditions_inform",
    label: "労働条件の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_welfare_benefits_consult",
    label: "福利厚生の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_welfare_benefits_inform",
    label: "福利厚生の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_workplace_communication_consult",
    label: "職場のコミュニケーションの相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_workplace_communication_inform",
    label: "職場のコミュニケーションの情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_subsidies_consult",
    label: "助成金の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_subsidies_inform",
    label: "助成金の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_care_services_consult",
    label: "介護サービスの相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_care_services_inform",
    label: "介護サービスの情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_workplace_environment_consult",
    label: "職場環境の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_workplace_environment_inform",
    label: "職場環境の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_skill_development_consult",
    label: "能力開発の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_skill_development_inform",
    label: "能力開発の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_employment_management_responsibility_consult",
    label: "雇用管理責任者関係の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_employment_management_responsibility_inform",
    label: "雇用管理責任者関係の情報",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_other_consult",
    label: "雇用管理に係るその他の相談",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "koyou_other_inform",
    label: "雇用管理に係るその他の情報",
    isDisplay: true,
    type: "checkbox",
  },
  // ... 能力開発に関する項目
  {
    name: "noukai_qualification_system_training_consult",
    label: "資格制度研修情報の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_qualification_system_training_inform",
    label: "資格制度研修情報の情報",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_job_posting_consult",
    label: "能力開発にかかる求人求職の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_job_posting_inform",
    label: "能力開発にかかる求人求職の情報",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_training_plan_curriculum_consult",
    label: "研修計画カリキュラム策定の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_training_plan_curriculum_inform",
    label: "研修計画カリキュラム策定の情報",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_subsidy_system_for_skill_development_consult",
    label: "能力開発にかかる助成制度関係の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_subsidy_system_for_skill_development_inform",
    label: "能力開発にかかる助成制度関係の情報",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_vocational_skill_development_promoter_consult",
    label: "職業能力開発推進者関係の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_vocational_skill_development_promoter_inform",
    label: "職業能力開発推進者関係の情報",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_other_skill_development_consult",
    label: "能力開発に係るその他の相談",
    isDisplay: false,
    type: "checkbox",
  },
  {
    name: "noukai_other_skill_development_inform",
    label: "能力開発に係るその他の情報",
    isDisplay: false,
    type: "checkbox",
  },
];

export const AUXILIARY_FIELDS = [
  {
    name: "_management_is_sanjo",
    label: "賛助会員",
    isDisplay: true,
    type: "checkbox",
  },
  {
    name: "_management_description",
    label: "管理情報",
    isDisplay: true,
    type: "text",
  },
  { name: "_jigyosyo_code", label: "事業所コード", isDisplay: true },
  { name: "_jigyosyo_type", label: "事業所タイプ", isDisplay: true },
  { name: "_jigyosyo_name", label: "事業所名", isDisplay: true },
  {
    name: "_jigyosyo_postal_code",
    label: "事業所郵便番号",
    isDisplay: true,
  },
  { name: "_jigyosyo_address", label: "事業所住所", isDisplay: true },
  { name: "_jigyosyo_tel_number", label: "事業所電話番号", isDisplay: true },
  { name: "_jigyosyo_fax_number", label: "事業所FAX番号", isDisplay: true },
  { name: "_jigyosyo_repr_name", label: "事業所代表者名", isDisplay: true },
  {
    name: "_jigyosyo_repr_position",
    label: "事業所代表者役職",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_kourou_url",
    label: "事業所公労ウェブサイトURL",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_kourou_release_datetime",
    label: "事業所公労リリース日時",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_number_of_member",
    label: "事業所会員数",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_exists_koyou_sekininsha",
    label: "事業所に雇用責任者が存在するか",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_is_use_kaigo_machine_subsidy",
    label: "介護機器助成金の使用",
    isDisplay: true,
  },
  {
    name: "_jigyosyo_is_use_other_subsidy",
    label: "その他の助成金の使用",
    isDisplay: true,
  },
];
